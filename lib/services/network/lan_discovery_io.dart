    ...
    try {
      ...
      _registration = await register(
        Service(
          ...
          port: _syncPort,
          txt: {
            'nodeId': Uint8List.fromList(utf8.encode(nodeId)),
            // Advertise the encrypted-sync capability so peers can pre-filter
            // old plaintext devices. The authoritative check is /sync/status.
            'proto': Uint8List.fromList(
              utf8.encode('${SyncCodec.protocolVersion}'),
            ),
          },
        ),
      );
