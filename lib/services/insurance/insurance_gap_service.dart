...
/// column). Soft-deleted items are excluded. Items with no `currentValueCents`
/// are excluded — we can't rank them.
class InsuranceGapService {
  final db_pkg.AppDatabase _db;
  InsuranceGapService(this._db);

  Future<List<Item>> topUncovered({int limit = 10, int minValueCents = 0}) async {
    final q = _db.select(_db.items)
      ..where((t) => t.isDeleted.equals(false))
      ..where((t) => t.isInsured.equals(false))
      ..where((t) => t.currentValueCents.isNotNull())
      ..where((t) => t.currentValueCents.isBiggerOrEqualValue(minValueCents))
      ..orderBy([
        (t) =>
            OrderingTerm(expression: t.currentValueCents, mode: OrderingMode.desc),
      ])
      ..limit(limit);
