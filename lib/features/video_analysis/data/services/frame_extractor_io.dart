...
/// Production frame stream for the orchestrator: extracts into a fresh
/// per-run temp subdirectory (so concurrent/successive runs never read
/// each other's `frame_*.png`) and cleans it up afterwards.
Stream<FrameData> extractVideoFrames({
  ...
  final dir = Directory(
    p.join(base.path, 'video_frames_${DateTime.now().microsecondsSinceEpoch}'),
  );
  await dir.create(recursive: true);
  try {
    yield* FrameExtractor(
      tempDir: dir.path,
    ).extractFrames(videoPath: videoPath, config: config);
  } finally {
    try {
      await dir.delete(recursive: true);
    } catch (_) {
      // Best-effort cleanup; stale temp frames are not worth a crash.
    }
  }
}

/// Longest edge (px) a decoded frame may have. VLM tiers get no benefit
/// from more, and the orchestrator buffers every extracted frame's bytes
/// for the run — native-resolution PNGs of a 1080p+ walkthrough (1-3 MB
/// each, hundreds per run) OOM-kill the app on Android.
const int kMaxFrameLongEdge = 1280;

/// The ffmpeg invocation for one extraction run. Pure so tests can pin
/// the memory-bounding pieces: fps sampling, the [kMaxFrameLongEdge]
/// downscale (aspect-preserving, never upscaling), and compressed JPEG
/// output instead of full PNGs.
String buildFrameExtractionCommand({
  required String videoPath,
  required double fps,
  required String outputPattern,
}) {
  // Single quotes protect the commas inside min() from the filtergraph
  // parser; force_original_aspect_ratio=decrease caps the LONG edge for
  // portrait video too and never upscales small ones.
  const scale =
      "scale='min($kMaxFrameLongEdge,iw)':'min($kMaxFrameLongEdge,ih)'"
      ':force_original_aspect_ratio=decrease';
  return '-i "$videoPath" -vf "fps=$fps,$scale" -vsync vfr -q:v 4 '
      '"$outputPattern"';
}
...
class FrameExtractor {
  ...
  Stream<FrameData> extractFrames({
    ...
    final outputPattern = p.join(_tempDir, 'frame_%05d.jpg');

    // Use ffmpeg to extract downscaled JPEG frames at the requested FPS.
